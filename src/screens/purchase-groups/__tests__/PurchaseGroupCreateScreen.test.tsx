...
import { PurchaseGroupCreateScreen } from '../PurchaseGroupCreateScreen';
...
jest.mock('@/repositories/PurchaseGroupRepository', () => ({
  PurchaseGroupRepository: jest.fn(),
}));
...
describe('PurchaseGroupCreateScreen', () => {
  test('1. renders with New Group title', () => {
  ...
  test('3. has save and cancel buttons', () => {
    const { getByTestId } = render(
      <NavigationContainer>
        <PurchaseGroupCreateScreen />
      </NavigationContainer>,
    ...
    expect(getByTestId('group-save-button')).toBeTruthy();
    expect(getByTestId('group-cancel-button')).toBeTruthy();
  });
});
